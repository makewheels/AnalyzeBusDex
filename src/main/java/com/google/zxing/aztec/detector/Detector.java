...
public final class Detector {
    ...
    private static class Point {
        ...
        public ResultPoint toResultPoint() {
            return new ResultPoint((float) this.f4345x, (float) this.f4346y);
        }
    ...
    private ResultPoint[] getMatrixCornerPoints(Point[] bullEyeCornerPoints) throws NotFoundException {
        ...
        int dx = bullEyeCornerPoints[0].f4345x - bullEyeCornerPoints[2].f4345x;
        dx += dx > 0 ? 1 : -1;
        int dy = bullEyeCornerPoints[0].f4346y - bullEyeCornerPoints[2].f4346y;
        dy += dy > 0 ? 1 : -1;
        int targetcx = round(((float) bullEyeCornerPoints[2].f4345x) - (((float) dx) * ratio));
        int targetcy = round(((float) bullEyeCornerPoints[2].f4346y) - (((float) dy) * ratio));
        int targetax = round(((float) bullEyeCornerPoints[0].f4345x) + (((float) dx) * ratio));
        int targetay = round(((float) bullEyeCornerPoints[0].f4346y) + (((float) dy) * ratio));
        dx = bullEyeCornerPoints[1].f4345x - bullEyeCornerPoints[3].f4345x;
        dx += dx > 0 ? 1 : -1;
        dy = bullEyeCornerPoints[1].f4346y - bullEyeCornerPoints[3].f4346y;
        dy += dy > 0 ? 1 : -1;
        int targetdx = round(((float) bullEyeCornerPoints[3].f4345x) - (((float) dx) * ratio));
        int targetdy = round(((float) bullEyeCornerPoints[3].f4346y) - (((float) dy) * ratio));
        int targetbx = round(((float) bullEyeCornerPoints[1].f4345x) + (((float) dx) * ratio));
        int targetby = round(((float) bullEyeCornerPoints[1].f4346y) + (((float) dy) * ratio));
    ...
    private Point[] getBullEyeCornerPoints(Point pCenter) throws NotFoundException {
        ...
        if (this.nbCenterLayers == 5 || this.nbCenterLayers == 7) {
            ...
            int dx = pina.f4345x - pinc.f4345x;
            int dy = pina.f4346y - pinc.f4346y;
            int targetcx = round(((float) pinc.f4345x) - (((float) dx) * ratio));
            int targetcy = round(((float) pinc.f4346y) - (((float) dy) * ratio));
            int targetax = round(((float) pina.f4345x) + (((float) dx) * ratio));
            int targetay = round(((float) pina.f4346y) + (((float) dy) * ratio));
            dx = pinb.f4345x - pind.f4345x;
            dy = pinb.f4346y - pind.f4346y;
            int targetdx = round(((float) pind.f4345x) - (((float) dx) * ratio));
            int targetdy = round(((float) pind.f4346y) - (((float) dy) * ratio));
            int targetbx = round(((float) pinb.f4345x) + (((float) dx) * ratio));
            int targetby = round(((float) pinb.f4346y) + (((float) dy) * ratio));
            if (isValid(targetax, targetay) && isValid(targetbx, targetby) && isValid(targetcx, targetcy) && isValid(targetdx, targetdy)) {
    ...
    private boolean[] sampleLine(Point p1, Point p2, int size) {
        ...
        float dx = (((float) (p2.f4345x - p1.f4345x)) * moduleSize) / d;
        float dy = (((float) (p2.f4346y - p1.f4346y)) * moduleSize) / d;
        float px = (float) p1.f4345x;
        float py = (float) p1.f4346y;
    ...
    private boolean isWhiteOrBlackRectangle(Point p1, Point p2, Point p3, Point p4) {
        Point p12 = new Point(p1.f4345x - 3, p1.f4346y + 3);
        Point p22 = new Point(p2.f4345x - 3, p2.f4346y - 3);
        Point p32 = new Point(p3.f4345x + 3, p3.f4346y - 3);
        Point p42 = new Point(p4.f4345x + 3, p4.f4346y + 3);
    ...
    private int getColor(Point p1, Point p2) {
        float d = distance(p1, p2);
        float dx = ((float) (p2.f4345x - p1.f4345x)) / d;
        float dy = ((float) (p2.f4346y - p1.f4346y)) / d;
        int error = 0;
        float px = (float) p1.f4345x;
        float py = (float) p1.f4346y;
        boolean colorModel = this.image.get(p1.f4345x, p1.f4346y);
    ...
    private Point getFirstDifferent(Point init, boolean color, int dx, int dy) {
        int x = init.f4345x + dx;
        int y = init.f4346y + dy;
    ...
    private static float distance(Point a, Point b) {
        return (float) Math.sqrt((double) (((a.f4345x - b.f4345x) * (a.f4345x - b.f4345x)) + ((a.f4346y - b.f4346y) * (a.f4346y - b.f4346y))));
    }
}
